...
public class DatePeriod { // what is record?
    ...
    private boolean doOverlap(DatePeriod period) {
        LocalDate firstPeriodStart = period.getFirstPeriod();
        LocalDate firstPeriodEnd = period.getSecondPeriod();
        return !firstPeriodStart.isAfter(getFirstPeriod()) && !firstPeriodEnd.isAfter(getSecondPeriod());
    }
